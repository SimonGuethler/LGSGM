...
# EfficientNet backbone to extract visual features
info_dict['visual_backbone'] = 'b5'
...
# Freeze all layers except the graph convolutional network and graph embedding module
info_dict['freeze'] = False


def run_evaluate(info_dict):
    # path to pretrained model
    info_dict['checkpoint'] = './Report/GCN_ObjAndPredShare_NoFtExModule_LSTM-02052024-175416.pth.tar'
    ...
    images_data = joblib.load(
        f"{DATA_DIR}/flickr30k_{subset}_lowered_images_data.joblib")
    caps_data = joblib.load(
        f"{DATA_DIR}/flickr30k_{subset}_lowered_caps_data.joblib")
    ...
    lossVal, ar_val, ari_val = evaluator.validate_retrieval(
        images_data, caps_data, True)
    info_txt = f"\n----- SUMMARY (Combine)-----\nLoss Val: {6-lossVal}"
    info_txt = info_txt + \
        f"\n[i2t] {round(ar_val[0], 4)} {round(ar_val[1], 4)} {round(ar_val[2], 4)}"
    info_txt = info_txt + \
